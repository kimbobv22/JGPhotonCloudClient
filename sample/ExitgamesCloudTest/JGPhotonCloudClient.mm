...
-(void)initializeClientWithUserName:(NSString *)userName_{
	...
	[Base setListener:self];
	_client = [[LoadBalancingClient alloc] initClient:self :jgVarJGPhotonCloudConfig_appID :jgVarJGPhotonCloudConfig_version :userName_ :jgVarJGPhotonCloudConfig_usingTcp];
...
- (void)debugReturn:(NSString*)string{
	if(!commonDelegate) return;
	[commonDelegate jgPhotonCloud:self debugReturn:DEBUG_LEVEL_ALL string:string];
}
...
-(BOOL)createRoomWithMaxPlayers:(uint)maxPlayers_ isVisible:(BOOL)isVisible_ isOpen:(BOOL)isOpen_ customProperties:(NSDictionary *)customProperties_ showPropertyKeys:(NSArray *)showPropertyKeys_{
	return [_client opCreateRoom:nil :isVisible_ :isOpen_ :maxPlayers_ :customProperties_ :nil :[EGArray arrayWithArray:showPropertyKeys_]];
}
