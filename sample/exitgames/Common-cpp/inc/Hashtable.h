...
namespace ExitGames
{
	namespace Common
	{
		...
		public:
			...
			JVector<Object> getKeys(void) const;
		...
		private:
			...
			private:
				template<typename Ftype> static Object stringArrayHelper(Ftype** pValue, short size){JString* pArr = new JString[size];for(short i=0;i<size;++i)pArr[i] = pValue[i];Object retVal= ValueObject<JString*>(pArr, size); delete pArr; return retVal;}
			};
		...
		/**
		   ...
		   @param key  Pointer to the key to return the corresponding value for.
		   @returns
