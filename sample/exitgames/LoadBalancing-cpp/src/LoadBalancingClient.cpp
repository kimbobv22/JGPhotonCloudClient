...
#	pragma warning(pop)
		...
		{
			EGLOG(DebugLevel::INFO, eventData.toString()); // don't print out the payload here, as that can get too expensive for big events
			...
			case EventCode::LEAVE:
				{
					...
					if(!mCurrentlyJoinedRoom.removePlayer(nr.getDataCopy()))
						EGLOG(DebugLevel::WARNINGS, L"LoadBalancingsuper::eventAction() EventCode::LEAVE - player %d who is leaving the room, has not been found in list of players, who are currently in the room", nr.getDataCopy());
		...
		void Client::debugReturn(Common::DebugLevel::DebugLevel debugLevel, const JString& string)
		{
		...
		void Client::readoutProperties(Hashtable& roomProperties, Hashtable& playerProperties, bool multiplePlayers, int targetPlayerNr)
		{
			...
			if(playerProperties.getSize())
			{
				...
				if(multiplePlayers)
				{
					const JVector<Object>& playersNrs = playerProperties.getKeys();
					unsigned int i=0;
