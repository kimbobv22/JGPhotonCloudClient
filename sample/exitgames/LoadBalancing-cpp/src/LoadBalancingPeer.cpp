...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		{
			return opRaiseEventImplementation(reliable, parameters, eventCode, channelID, targetPlayers, numTargetPlayers, EVC_DO_NOT_CACHE, RG_OTHERS);
		}
		...
		bool Peer::opAuthenticate(const JString& appID, const JString& appVersion, bool encrypted)
		{
			...
			PhotonPeer_sendDebugOutput(this, DEBUG_LEVEL_INFO, OperationRequest(OperationCode::AUTHENTICATE, op).toString(true));
		...
		{
			OperationRequestParameters op;
			op.put(P_PROPERTIES, ValueObject<Hashtable>(properties));
			op.put(P_ACTORNR, ValueObject<nByte>(playerNr));
			
			bool broadcast = true;
			op.put(P_BROADCAST, ValueObject<bool>(broadcast));
		...
		{
			Hashtable stripped;
			JVector<Object> keys = orig.getKeys();
		...
		{
			OperationRequestParameters op;
			op.put(P_DATA, ValueObject<Hashtable>(parameters));
			op.put(P_CODE, ValueObject<nByte>(eventCode));
			
			if(targetPlayers)
				op.put(P_ACTOR_LIST, ValueObject<int*>(targetPlayers, numTargetPlayers));
			else
			{
				if(eventCaching != EVC_DO_NOT_CACHE)
					op.put(P_CACHE, ValueObject<nByte>(eventCaching));
				if(receiverGroup != RG_OTHERS)
					op.put(P_RECEIVER_GROUP, ValueObject<nByte>(receiverGroup));
			}
